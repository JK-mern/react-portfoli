import React from "react";

function Skills() {
  return (
    <div className="max-w-6xl mx-auto my-10" id="Skills">
      <div className="flex flex-col">
        <div>
          <h1 className="text-center font-bold text-3xl text-black underline">
            SKILLS
          </h1>
        </div>
        <div className="  flex max-w-screen-sm md:max-w-full mx-auto flex-row justify-center items-center gap-14 flex-wrap mt-16 p-6">
          <div className="flex flex-col text-center w-1/3 md:w-1/12">
            <img
              src="https://raw.githubusercontent.com/devicons/devicon/6910f0503efdd315c8f9b858234310c06e04d9c0/icons/html5/html5-original.svg"
              alt=""
              className="w-14 self-center"
            />
            <h3 className="mt-3">HTML</h3>
          </div>
          <div className="flex flex-col text-center  w-1/3  md:w-1/12">
            <img
              src="https://raw.githubusercontent.com/devicons/devicon/6910f0503efdd315c8f9b858234310c06e04d9c0/icons/css3/css3-original.svg"
              alt=""
              className="w-14 self-center"
            />
            <h3 className="mt-3">CSS</h3>
          </div> 
          <div className="flex flex-col text-center w-1/3  md:w-1/12">
            <img
              src="https://raw.githubusercontent.com/devicons/devicon/6910f0503efdd315c8f9b858234310c06e04d9c0/icons/javascript/javascript-original.svg"
              alt=""
              className="w-14 self-center"
            />
            <h3 className="mt-3">JavaScipt</h3>
          </div>
          <div className="flex flex-col text-center  w-1/3 md:w-1/12">
            <img
              src="https://raw.githubusercontent.com/devicons/devicon/6910f0503efdd315c8f9b858234310c06e04d9c0/icons/react/react-original.svg"
              alt=""
              className="w-14 self-center"
            />
            <h3 className="mt-3">React</h3>
          </div>

          <div className="flex flex-col text-center  w-1/3 md:w-1/12">
            <img
              src="  https://raw.githubusercontent.com/devicons/devicon/6910f0503efdd315c8f9b858234310c06e04d9c0/icons/tailwindcss/tailwindcss-original.svg"
              alt=""
              className="w-14  self-center"
            />
            <h3 className="mt-3">Tailwind </h3>
          </div>
 
          <div className="flex flex-col text-center  w-1/3 md:w-1/12" >
            <img
              src="https://raw.githubusercontent.com/devicons/devicon/6910f0503efdd315c8f9b858234310c06e04d9c0/icons/redux/redux-original.svg"
              alt=""
              className="w-14  self-center"
            />
            <h3 className="mt-3">Redux</h3>
          </div>

          <div className="flex flex-col text-center  w-1/3 md:w-1/12">
            <img
              src="https://raw.githubusercontent.com/devicons/devicon/6910f0503efdd315c8f9b858234310c06e04d9c0/icons/vitejs/vitejs-original.svg"
              alt=""
              className="w-14  self-center"
            />
            <h3 className="mt-3">VITE</h3>
          </div>

          <div className="flex flex-col text-center w-1/3 md:w-1/12">
            <img
              src="https://raw.githubusercontent.com/devicons/devicon/6910f0503efdd315c8f9b858234310c06e04d9c0/icons/axios/axios-plain.svg"
              alt=""
              className="w-14  self-center"
            />
            <h3 className="mt-3">Axios</h3>
          </div>

          <div className="flex flex-col text-center w-1/3 md:w-1/12">
            <img
              src="https://raw.githubusercontent.com/devicons/devicon/6910f0503efdd315c8f9b858234310c06e04d9c0/icons/reactrouter/reactrouter-original.svg"
              className="w-10  self-center"
            />
            <h3 className="mt-3 text-base">React Router Dom</h3>
          </div>

          <div className="flex flex-col text-center w-1/3 md:w-1/12">
            <img
              src="https://raw.githubusercontent.com/devicons/devicon/6910f0503efdd315c8f9b858234310c06e04d9c0/icons/express/express-original.svg"
              alt=""
              className="w-14  self-center"
            />
            <h3 className="mt-3">Express</h3>
          </div>

          <div className="flex flex-col text-center w-1/3 md:w-1/12">
            <img
              src="https://raw.githubusercontent.com/devicons/devicon/6910f0503efdd315c8f9b858234310c06e04d9c0/icons/mongodb/mongodb-original.svg"
              alt=""
              className="w-14  self-center"
            />
            <h3 className="mt-3">Mongo DB</h3>
          </div>

          <div className="flex flex-col text-center w-1/3 md:w-1/12">
            <img
              src="https://raw.githubusercontent.com/devicons/devicon/6910f0503efdd315c8f9b858234310c06e04d9c0/icons/mongoose/mongoose-original.svg"
              alt=""
              className="w-14  self-center"
            />
            <h3 className="mt-3">Moongoose</h3>
          </div>

          <div className="flex flex-col text-center w-1/3 md:w-1/12">
            <img
              src="https://raw.githubusercontent.com/devicons/devicon/6910f0503efdd315c8f9b858234310c06e04d9c0/icons/nodejs/nodejs-original.svg"
              alt=""
              className="w-14  self-center"
            />
            <h3 className="mt-3">Nodejs</h3>
          </div>

          <div className="flex flex-col text-center w-1/3 md:w-1/12">
            <img
              src="https://raw.githubusercontent.com/devicons/devicon/6910f0503efdd315c8f9b858234310c06e04d9c0/icons/nodemon/nodemon-original.svg"
              alt=""
              className="w-14  self-center"
            />
            <h3 className="mt-3">Nodemon</h3>
          </div>

          <div className="flex flex-col text-center w-1/3 md:w-1/12">
            <img
              src="https://raw.githubusercontent.com/devicons/devicon/6910f0503efdd315c8f9b858234310c06e04d9c0/icons/npm/npm-original-wordmark.svg"
              alt=""
              className="w-14  self-center"
            />
            <h3 className="mt-3">Npm</h3>
          </div>

          <div className="flex flex-col text-center w-1/3 md:w-1/12">
            <img
              src="https://raw.githubusercontent.com/devicons/devicon/6910f0503efdd315c8f9b858234310c06e04d9c0/icons/git/git-original.svg"
              alt=""
              className="w-14  self-center"
            />
            <h3 className="mt-3">Git</h3>
          </div>

          <div className="flex flex-col text-center w-1/3 md:w-1/12">
            <img
              src="https://raw.githubusercontent.com/devicons/devicon/6910f0503efdd315c8f9b858234310c06e04d9c0/icons/github/github-original.svg"
              alt=""
              className="w-14  self-center"
            />
            <h3 className="mt-3">Github</h3>
          </div>

          <div className="flex flex-col text-center  w-1/3 md:w-1/12">
            <img
              src="https://raw.githubusercontent.com/devicons/devicon/6910f0503efdd315c8f9b858234310c06e04d9c0/icons/ubuntu/ubuntu-original.svg"
              alt=""
              className="w-14  self-center"
            />
            <h3 className="mt-3">Ubuntu</h3>
          </div>

          <div className="flex flex-col text-center w-1/3 md:w-1/12">
            <img
              src="https://raw.githubusercontent.com/devicons/devicon/6910f0503efdd315c8f9b858234310c06e04d9c0/icons/vscode/vscode-original.svg"
              alt=""
              className="w-14  self-center"
            />
            <h3 className="mt-3">Vs Code</h3>
          </div>
        </div>
      </div>
    </div>
  );
}

export default Skills;
